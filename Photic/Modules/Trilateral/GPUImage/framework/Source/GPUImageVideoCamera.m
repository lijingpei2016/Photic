...
const GLfloat kJPColorConversion[] = {
    1.0,      0.0,        1.0,
    0.0,      -0.3337633, 1.732446,
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
                if ([connection respondsToSelector:@selector(setVideoMinFrameDuration:)]) connection.videoMinFrameDuration = CMTimeMake(1, _frameRate);

                if ([connection respondsToSelector:@selector(setVideoMaxFrameDuration:)]) connection.videoMaxFrameDuration = CMTimeMake(1, _frameRate);
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
                if ([connection respondsToSelector:@selector(setVideoMinFrameDuration:)]) connection.videoMinFrameDuration = kCMTimeInvalid;  // This sets videoMinFrameDuration back to default

                if ([connection respondsToSelector:@selector(setVideoMaxFrameDuration:)]) connection.videoMaxFrameDuration = kCMTimeInvalid;  // This sets videoMaxFrameDuration back to default
#pragma clang diagnostic pop
            }
        }
    }
...
- (void)tappedToFocusAtPoint:(CGPoint)point {
    ...
    if (device.isFocusPointOfInterestSupported &&
        [device isFocusModeSupported:AVCaptureFocusModeContinuousAutoFocus]) {//先进行判断是否支持控制对焦
...
- (void)setExposurePoint:(CGPoint)exposurePoint {
    ...
    if ([device isExposurePointOfInterestSupported]) {
        ...
        if ([device lockForConfiguration:&error]) {
            [device setExposurePointOfInterest:adjustedPoint];
            [device unlockForConfiguration];
        } else {
            NSLog(@"jp ---------------------->setExposurePoint 、lockForConfiguration");
        }
    } else {
        NSLog(@"jp ---------------------->setExposurePoint");
    }
}
...
- (void)setCameraZoomFactor:(CGFloat)zoom {
    AVCaptureDevice *device = self.inputCamera;
    zoom = MAX(zoom, 1.0);
    zoom = MIN(zoom, device.activeFormat.videoMaxZoomFactor);
...
{
    return [self.inputCamera.activeFormat isVideoStabilizationModeSupported:videoStabilizationMode];
}

- (void)setVideoStabilizationMode:(AVCaptureVideoStabilizationMode)videoStabilizationMode API_AVAILABLE(ios(8.0))
{
    if ([self.inputCamera.activeFormat isVideoStabilizationModeSupported:videoStabilizationMode]) {
        AVCaptureConnection *videoConnection = [self->videoOutput connectionWithMediaType:AVMediaTypeVideo];
        [videoConnection setPreferredVideoStabilizationMode:videoStabilizationMode];
        self.videoStabilizationMode = videoStabilizationMode;
    } else {
        NSLog(@"jp ---------------------->setVideoStabilizationMode:videoStabilizationMode");
    }
}
...
- (void)isSmoothAutoFocusEnabledWith:(BOOL)isEnabled {
    ...
    if (device.isSmoothAutoFocusEnabled) {
        NSError *error;
        if ([device lockForConfiguration:&error]) {
            device.smoothAutoFocusEnabled = isEnabled;
            [device unlockForConfiguration];
        }
    }
}
